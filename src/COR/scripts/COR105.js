 ...
 * @author     SC-PROSOFT
 * @descrip    Release Electron - COR105 RM Version / .
 *             Guarda actualizacion unidades funcionales .
 **/
...
new Vue(
    {
        ...
        methods: {
            async init() {
                ...
                nombreOpcion('1,5 - Actualizar unidades funcionales servicios')
                $this.listado_undfuncionales = await get('und_funcionales');
                $this.listado_undfuncionales = $this.listado_undfuncionales['unidades'];
            ...
            evaluar_novedad(novedad) {
                ...
                if ($this.listado_undfuncionales) {
            ...
            validar_codigo() {
                validarInputs(
                    { form: "#codigo_cor105", orden: "1" },
                    function volver() { $this.listado_undfuncionales ? (_inputControl('disabled'), CON850($this.evaluar_novedad)) : $this.salir_opcion(); },
                    function avanzar() {
                        $this.actual['codigo'] = $this.form['codigo'];
                        const consulta = $this.listado_undfuncionales.find(x => parseInt(x.codigo) === parseInt($this.actual['codigo']));
            ...
            validar_descripcion() {
                validarInputs(
                    { form: "#descripcion_cor105", orden: "1" },
            ...
            /*------------------- V E N T A N A S ( F8 )------------------------*/
            ventana_unidades_funcionales() {
                _ventanaDatos(
                    {
                        titulo: "Ventana unidades funcionales",
                        columnas: ["codigo", "descripcion"], label: ["Código ", "Descripción"],
                        callback_esc: () => $this.validar_codigo(),
                        callback: (data) => {
                            $this.form['codigo'] = `${data.codigo}`;
                            $this.form['descripcion'] = `${data.descripcion}`;
                            _enterInput(`#input_codigo_cor105`);
                        },
                        data: $this.listado_undfuncionales
                    }
                );
            },
            /*----------------------- R U T I N A S ----------------------------*/
            ...
            grabar_actualizacion_registro() {
                ...
                postData(data_envio, get_url("APP/COR/CORR105.DLL"))
                    .then(data => {
