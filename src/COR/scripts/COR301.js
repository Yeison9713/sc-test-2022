...
new Vue({
    ...
    created() {
        ...
        nombreOpcion('3-1 - Informe de correspondencia');
        _inputControl('reset');
    ...
    methods: {
        async get_json() {            
            ...
            get('terceros').then(terceros => _this.terceros = terceros.TERCEROS)
        },
        datos_fijos() {
            this.reg.nit_w = '99'
            document.getElementById('nombre_entidad_COR301').value = 'TODAS LAS ENTIDADES'

            this.reg.dep_w = '***'
            document.getElementById('descripcion_dep_COR301').value = 'TODAS LAS DEPENDENCIAS'
        ...
        solicitar_formato() {
            POPUP({
                ...
                seleccion: this.formato.id || '1',
                array: [
                    { id: 1, label: 'Formato PDF' },
                    { id: 2, label: 'Formato EXCEL' },
                ],
                callback_f: _toggleNav,
            }, (data) => {
                this.formato = data
                this.format_formato = `${data.id} - ${data.label}`
                        ...
                        if(nit && nit.toString().trim()) {
                            ...
                            if(nit == '99'){
                                ...
                                if (!busqueda) {
                                    ...
                                    document.getElementById('nombre_entidad_COR301').value = busqueda.NOMBRE    
                                    _this.validar_dependencia()
                                }
                            }
            ...
            validarInputs({
                ...
                () => {
                    ...
                    if (!busqueda && servicio != '***') {
                        ...
                        let envio_final = () => {
                            if (servicio == '***') document.getElementById('descripcion_dep_COR301').value = 'TODAS LAS DEPENDENCIAS'
                            else document.getElementById('descripcion_dep_COR301').value = busqueda.responsable
            ...
            validarInputs({
                ...
                () => {
                    ...
                    if (!busqueda_tipo && tipo != '**') {
                        ...
                        if (tipo == '**') document.getElementById('descrip_tipo_COR301').value = 'TODOS LOS TIPOS'
                        else document.getElementById('descrip_tipo_COR301').value = busqueda_tipo.descripcion
        ...
        validar_descartar() {
            POPUP({
                ...
                setTimeout(this.validar_respuesta, 300)
            })
        },
        validar_respuesta() {
            POPUP({
                titulo: "INCLUIR RESPUESTA",
                indices: [{ id: 'id', label: 'label' }],
                seleccion: this.reg.respuesta.id || 'N',
                array: [
                    { id: 'S', label: 'SI' },
                    { id: 'N', label: 'NO' },
                ],
                teclaAlterna: true,
                callback_f: () => { setTimeout(this.validar_descartar), 300 },
            }, (data) => {
                this.reg.respuesta = data
                this.format_repuesta = `${data.id} - ${data.label}`
        ...
        formato_impresion(data = [], fechas = {}) {
            ...
            var data_format = data.map(el => {
                ...
                el.estado = format_estado(el.estado)
            ...
            let respuesta = this.reg.respuesta.id == 'S'
            ...
            let width_tabla_pdf = ['auto', 'auto', 'auto', '*', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto']
            if (respuesta) {
                header_pdf.push(
                    {
                        text: 'Nro. resp.',
                        title: 'Nro. resp.',
                        value: 'radicado_salida',
                        format: 'string'
                    },
                    {
                        text: 'Fecha resp.',
                ...
                width_tabla_pdf.push('auto', 'auto')
            ...
            header_pdf.push({ text: 'Firma', title: 'Firma' })

            let header_tabla_pdf = header_pdf.map(el => {
            ...
            data_format.forEach(el => {
                let obj = [
                    { text: el.radicado_entrada, },
                    { text: el.fecha_format },
                    { text: el.hora_format },
                    { text: el.descrip_ter },
                    { text: el.descrip_tipco },
                    { text: el.descripcion },
                    { text: el.descrip_serco },
                    { text: el.responsable },
                    { text: el.folio_format },
                    { text: el.manejo.substring(0, 4) },
                    { text: el.procedencia.substring(0, 4) },
                ]

                if (respuesta) {
                    obj.push({ text: el.radicado_salida }, { text: el.fecha_resp })
                }
                obj.push({ text: '' })
                            ...
                            columns: [
                                ...
                                },
                                {
                                    ...
                                    text: [
                                        ...
                                        {
                                            text: 'CONTROL DE CORRESPONDENCIA\n',
                                            style: 'subTitulo'
                                        },
                                        {
            ...
            if (formato == 1) {
                ...
                config = {
                    ...
                    tipo: 'excel',
                    logo: `${nit}.png`,
                    header: [empresa.toUpperCase(), 'CONTROL DE CORRESPONDENCIA', `FECHA: ${fecha_ini} - ${fecha_fin}`],
                    tabla: {
                        columnas: header_excel,
                        data: data_format
                    }
                }
            }
