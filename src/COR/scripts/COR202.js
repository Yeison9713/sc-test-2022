const { get, CORR201P, init_datos_w } = require('../../COR/scripts/COR.ctrl.js')
...
new Vue({
    ...
    mounted() {
        this.monto_mask = new IMask(
            this.$refs.monto_input_COR202,
            { mask: Number, min: -999999999999, max: 999999999999, scale: 2, thousandsSeparator: ',', radix: '.' }
        )
    },
    created() {
        console.clear()
        ...
        nombreOpcion('2-2 - Reimprimir correspondencia');
        _inputControl('reset');
    ...
    methods: {
        ...
        cambio(data) {
            ...
            let serv_busc = _this.servicios.find(el => el.codigo.padStart(3, '0') == serv_w)
            ...
            document.getElementById('nombre_remitente_COR202').value = data.descrip_ter || ''
            document.getElementById('descrip_depto_remit_COR202').value = dptoremi_busc ? dptoremi_busc.descripcion : ''
            document.getElementById('descrip_tipo_corr_COR202').value = tipo_corres_busc ? tipo_corres_busc.descripcion : ''
            document.getElementById('descrip_aux_corr_COR202').value = cod_auxco_busc ? cod_auxco_busc.descripcion : ''
            document.getElementById('descrip_servicio_COR202').value = serv_busc ? serv_busc.descripcion : ''
            document.getElementById('descrip_personal_COR202').value = dep_busc ? dep_busc.descripcion : ''
            _this.bb(data)
        },
        bb(data) {
            const _this = this;
        ...
        impresion(data) {
            ...
            let descrip_ter = document.getElementById('nombre_remitente_COR202').value
            let descrip_dep_w = document.getElementById('descrip_personal_COR202').value
            let descrip_tipo_w = document.getElementById('descrip_tipo_corr_COR202').value
            let descrip_servicio_w = document.getElementById('descrip_servicio_COR202').value
            ...
            loader('show')
            CORR201P(dato_impresion)
                .then(() => {
                    loader('hide')
                    setTimeout(_this.validar_radicado, 500)
                })
                .catch(() => {
                    loader('hide')
                    setTimeout(_this.validar_radicado, 500)
        ...
        ventana_correspondencia() {
            ...
            _ventanaDatos({
                ...
                callback_esc: function () {
                    document.getElementById('radicado_COR202').focus()
                },
                callback: function (data) {
                    _this.reg.cont_w = data['cont_llave'];
                    _this.reg.ano_llave_w = data['ano_llave'];
                    _enterInput('#radicado_COR202');
                }
            });
        },
        reset_form() {
