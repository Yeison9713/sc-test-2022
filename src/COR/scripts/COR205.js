const { get, enviar_correspondencia, init_datos_w } = require('../../COR/scripts/COR.ctrl.js')
...
new Vue({
    ...
    created() {
        ...
        nombreOpcion('2-5 - Envio correspondencia pendiente');
        _inputControl('reset');
    ...
    methods: {
        ...
        cambio(data) {
            ...
            document.getElementById('nombre_remitente_COR205').value = data.descrip_ter || ''
            document.getElementById('descrip_depto_remit_COR205').value = dptoremi_busc ? dptoremi_busc.descripcion : ''
            document.getElementById('descrip_tipo_corr_COR205').value = tipo_corres_busc ? tipo_corres_busc.descripcion : ''
            document.getElementById('descrip_aux_corr_COR205').value = cod_auxco_busc ? cod_auxco_busc.descripcion : ''
            document.getElementById('descrip_servicio_COR205').value = serv_busc ? serv_busc.descripcion : ''
            document.getElementById('descrip_personal_COR205').value = dep_busc ? dep_busc.descripcion : ''
            _this.bb(data)
        },
        bb(data) {
            const _this = this;
        ...
        envio_email(data) {
            ...
            let destino_info = _this.dependencias.find(el => el.codigo.padStart(3, '0') == destino)

            if (!destino_info) {
                CON851('01', 'No se ha encontrado el personal de destino', null, 'error', 'Advertencia');
                _this.validar_radicado()
            } else if (!destino_info.correo.trim()) {
                CON851('', '2K', null, 'error', 'Advertencia');
                _this.validar_radicado()
            } else {
                ...
                let error_email = (error) => {
                    ...
                    CON851('99', 'Ha ocurrido un error intentando enviar la notificación por correo eletrónico.', null, 'error', 'Advertencia');
                    _this.validar_radicado(data)
                }
                ...
                enviar_correspondencia(listado)
                    .then(res => {
                        ...
                        if (res.code != 0) error_email(res)
                            ...
                            CON851('', 'Se ha enviado la notificación correctamente.', null, 'success', 'Correcto');
                            _this.validar_radicado(data)
                        }
                    })
                    .catch(error_email)
        ...
        ventana_correspondencia() {
            ...
            _ventanaDatos({
                ...
                callback_esc: function () {
                    document.getElementById('radicado_COR205').focus()
                },
                callback: function (data) {
                    _this.reg.cont_w = data['cont_llave'];
                    _this.reg.ano_llave_w = data['ano_llave'];
                    _enterInput('#radicado_COR205');
                }
            });
        },
        reset_form() {
