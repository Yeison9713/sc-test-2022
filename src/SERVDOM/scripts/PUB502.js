// Listado de facturacion x Codigo - Opcion 5-2 - David.M - 05/08/2021
...
new Vue({
  ...
  created() {
    nombreOpcion("5-2 - Listado validación facturas");
    _inputControl("reset");
  ...
  methods: {
    ...
    llamarDLL() {
      CON851P("00", this.datoFechaArchivo, () => {
        ...
        postData({ datosh: moduloDatosEnvio() + datos_envio.join("|") + "|" }, get_url("app/SERVDOM/PUB502.DLL"), {
          onProgress: (progress) => {
    ...
    imprimir(data) {
      ...
      if (listado.length < 1) {
        ...
        header_format = [
          ...
          `VALIDACIÓN DE FACTURACIÓN - ${this.descrip_serv} - ${this.ano_gen}/${this.mes_gen} NIT: ${nit_edit}`,
