// Actualizacion de lecturas - 2-7 - David.M - 26/07/2021
...
var _this = new Vue({
  ...
  created() {
    nombreOpcion("2-7 - Actualización de Lecturas");
    _inputControl("reset");
  ...
  methods: {
      ...
      validarInputs(
        ...
        () => {
          ...
          if (!busqueda) {
            ...
            if (this.servicio == 2 || (this.servicio == 4 && $_USUARIO_EMPRESA.NIT != 892099475)) {
              CON851("", "01", null, "error", "Error");
              this.datoServicio();
            } else if (this.sw_nuevos == 0) {
              this.sw_nuevos = 1;
              loader("hide");
              postData(
                { datosh: moduloDatosEnvio(), fecha: this.nom_fac, serv: this.servicio },
                get_url("app/SERVDOM/PUB207N.DLL")
              )
                .then((data) => {
                  console.log(data, " PUB207N");
                  loader("hide");
                ...
                .catch((error) => {
                  console.error(error);
                  CON851("", "Error, intentelo nuevamente", null, "error", "Error");
                  loader("hide");
                  this.datoServicio();
                });
            } else {
    ...
    async cambio() {
      ...
      if (this.reg_fact.tabla.serv_tab[this.serv_w].lec_ant_med == 0) {
        this.datoAct();
      } else if (this.reg_fact.tabla.serv_tab[this.serv_w].lec_act_med > 0) {
        this.datoPrx();
      } else {
        this.datoAnt();
      }
    ...
    datoAnt() {
      if (parseInt($_USUARIO_EMPRESA.NIT) == 892099475) {
        // Villanueva - se omite lect ant
        this.datoAct();
      } else {
        validarInputs(
          {
    ...
    depurarNovedad() {
      ...
      console.log(this.consumo_w, "consumo_w");
      console.log(this.consumo_est_w, "consumo_est_w");
