new Vue({
    ...
    created() {
        ...
        nombreOpcion('5 - Listado por fecha rayos x')
    ...
    methods: {
        ...
        validarDiaPOS_lablis() {
            validarInputs(
                ...
                () => {
                    ...
                    } else this.validarMostrarContenido()
                }
            )
        },
        validarMostrarContenido(){
            validarInputs(
                {
                  form: "#validarContenido_rxlis",
                  orden: "1",
                },
                () => this.validarDiaPOS_lablis(),
                () => {
                  this.mostrarContenido =
                    this.mostrarContenido.toUpperCase().trim() != "S" ? "N" : "S";
        ...
        llamadoDLL() {
            loader('show');
            postData({ datosh: datosEnvio() + this.anoListado + this.mesPREListado + this.diaPREListado + '|' + this.anoListado + this.mesPOSListado + this.diaPOSListado + '|' + this.mostrarContenido + '|' }, get_url('app/RX/RX-500.DLL'))
                .then(data => {
                    loader('hide');
                    _this._montarImpresion_RXLIS(data);
                })
                .catch(err => {
                    console.log(err)
                    loader('hide');
