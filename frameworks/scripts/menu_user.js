...
$(function () {
    $('.header-v2').attr('style', `background: url(http://localhost/sc-test-2022/src/imagenes/header-${localStorage.Modulo.trim().toLowerCase()}.webp)`)
});
...
const _cargarScripts = () => {
    ...
    $.ajax({
        ...
        // if (modulo == 'SAL' || modulo == 'RX' || modulo == 'LAB' || $_USUA_GLOBAL[0].TIPO_EMPRE == 'H') { // Esperando verificación - YO
...
function _toggleNav() {
    ...
    if (widthScreen > 992) {
        if (visible) {
            ...
            if (modulo != "HIC")
                $("#body_main").html(
                    '<img src="../../imagenes/PENSADOR SUELTO-lite.png" style="width: 400px;">'
                );
