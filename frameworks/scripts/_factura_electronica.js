...
class _req_FACT {
  ...
  async get_dataJson(data) {
    var arrayData = [], textoPlano_XML = '', respuesta_json = ''
      ...
      switch (parseInt(proveedor)) {
        ...
        case 4://EKOMERCIO
          ...
          for (var i in arrayData) {
            ...
            toastr.success(`Enviando factura: ${splitText[26].concat(nro_fact)}`, `Envia: ${splitText[1]} Recibe: ${splitText[41]} `);

            const response = await req.send_request(data_fact);
            res_DIAN = response;
            const excluir = `<![CDATA[<?xml version="1.0" encoding="utf-8" standalone="no"?><ApplicationResponse xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:sts="dian:gov:co:facturaelectronica:Structures-2-1" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns="urn:oasis:names:specification:ubl:schema:xsd:ApplicationResponse-2"`;

            const response_xml = await req.parseXML_json(response);
            const response_body = response_xml['Envelope']['Body'][0]['procesarTextoPlanoResponse'][0]['procesarTextoPlanoResult'][0];

            let json_body = JSON.stringify(response_body, function (key, value) {
              if (typeof value === "function") return undefined;
              return value;
            });

            facturas_procesadas.push({ "factura": data_fact.ID, "body": json_body, "estado": null, 'errores': null, 'advertencias': null })
        ...
        case 1://FACSE
          ...
          for (var i in arrayData) {
            ...
            const nro_fact = data_fact['Comprobante']['Prefijo'].concat(data_fact['Comprobante']['Numero'].toString().trim().padStart(6, '0'));

            toastr.success(`Enviando factura: ${data_fact['Comprobante']['Numero'].toString().padStart(6, '0')} Envia: ${data_fact['Emisor']['RazonSocial']} Recibe: ${data_fact['Receptor']['RazonSocial']} `);
        ...
        case 5://EMISION
          ...
          for (var i in arrayData) {
            ...
            toastr.success(`Enviando factura: ${(data_fact['number'].toString().trim()).padStart(6, '0')} Envia: ${nit_envia} Recibe: ${data_fact['customer']['name']} `);
            ...
            if (response.status == 401) {
              toastr.info((req.prueba) ? 'El TOKEN usado no corresponde al tipo de servicio de la petición: envio_prueba\n' : 'El TOKEN usado no corresponde al tipo de servicio de la petición: envio_produccion\n');
            } else {
              toastr.info((req.prueba) ? 'El TOKEN usado no corresponde al tipo de servicio de la petición: envio_prueba\n' : 'El TOKEN usado no corresponde al tipo de servicio de la petición: envio_produccion\n');
  ...
  async procesarRespuesta(res_XML, indice, date) {
    let dataJson = new Object();
    /**********FACTURA RECHAZADA***********/
    if (res_XML.indexOf('Error') != -1) {//Se encontraron Errores
      const resultado = JSON.parse(res_XML);
      let errores = resultado['Error'], data_errores = [];
      ...
      for (const i in errores) {
        ...
        data_errores.push({ CODE: code, MESSAGE: mensaje_error, DATE: date_error });
        ...
        dataJson = {
          ...
          "RESPUESTA": JSON.stringify(data_errores),
          "PROVEEDOR": 'EKOMERCIO'
        }
      ...
      /**********FACTURA APROBADA***********/
      const resultado = JSON.parse(res_XML);
      let objResultado = resultado['AttachedDocument'][0];

      if (typeof objResultado == 'undefined') err_proveedor = true;
      else {
        err_proveedor = false;
        let descrip = objResultado['ParentDocumentLineReference'][0];
        const dirDelete = facturas_data[indice]['DIR'] + '\\' + 'GENERADOS\\' + facturas_data[indice]['ID'] + '.txt'
        fs.unlink(dirDelete, function (err) { if (err) throw err; });

        let description = descrip['DocumentReference'][0]['Attachment'][0]['ExternalReference'][0]['Description'][0];
        let lineResponse = await req.parseXML_json({ status: 200, data: description });
        let line_cufe = lineResponse['ApplicationResponse']['DocumentResponse'][0]['DocumentReference'][0]['UUID'][0];
        const cufe = { ESQUEMA: line_cufe['ATTR'].schemeName, CODE: line_cufe['CODE'] }
        lineResponse = lineResponse['ApplicationResponse']['DocumentResponse'][0]['LineResponse'];

        let lineas = [];

        lineas = await lineResponse.filter((linea) => {
          const val = linea['Response'][0]['Description'][0];
          if (val != "0 " || typeof val != 'undefined') return linea;
        })

        let warnings = lineas.map((arr) => {
          const response_linea = arr['Response'][0]['Description'][0];
          const response_code = arr['Response'][0]['ResponseCode'][0];
          const encodeText = Buffer.from(response_linea, 'ascii');
          const parseText = encodeText.toString('utf8');

          if (parseText != "0" || typeof parseText != 'undefined') {
            return { CODE: response_code, MESSAGE: parseText }
          };
  ...
  async crearLogs_Fact(dataFact, indice) {
    let dateTime = new Date(), dataLog = '', dataJson = new Object();
    ...
    const fecha = dateTime.getDate().toString().padStart(2, '0') + '-' + mes.padStart(2, '0') + '-' + (dateTime.getFullYear().toString());
    ...
    switch (parseInt(req.proveedor)) {
      ...
      case 1://FACSE
        ...
        if (!req.nota_credito && mensaje.cufe['Respuesta'] == true) {
          if (req.contabilidad == null) {
            ...
            req.crearTxt_LOGS({
              'id': indice, 'ruta': rutalog,
              'plano': data_plano, 'cufe': facturas_logs[indice]['CUFE']
            });
      ...
      case 4: //EKOMERCIO
        ...
        if (!err_proveedor) {
          ...
          if (!req.nota_credito && dataJson['ESTADO_ENVIO'].split('|')[0] == '00') {

            if (req.contabilidad == null) {

              dataLog += facturas_data[indice].PREFIJO + '|';
              dataLog += facturas_procesadas[indice].NRO + '|';
              dataLog += (dateTime.getDate().toString()) + '|'
              dataLog += mes.padStart(2, '0') + '|'
              dataLog += (dateTime.getFullYear().toString()) + '|'
              dataLog += facturas_logs[indice]['CUFE'].CODE + '|'
              dataLog += (dateTime.getHours().toString()).padStart(2, '0') + ':'
              dataLog += (dateTime.getMinutes().toString()).padStart(2, '0');
              ...
              req.crearTxt_LOGS({
                'id': indice, 'ruta': rutalog,
                'plano': data_plano, 'cufe': facturas_logs[indice]['CUFE'].CODE
              });
      ...
      case 5:// EMISION
        ...
        if (resData['status'] == 'error') {//  FACTURA RECHAZADA
          ...
          if (errores['MESSAGE'][0].indexOf('Regla: 90') != -1 && !req.nota_credito) {
            if (req.contabilidad == null) {
              ...
              req.crearTxt_LOGS({
                'id': indice, 'ruta': rutalog,
                'plano': data_plano, 'cufe': errores['MESSAGE'][0].split("'")[1]
              });
  ...
  crearTxt_LOGS(data) {
    ...
    xml = xml.join('>');
    xml = xml.replace(/[\r\n]+/g, "\n");
    fs.writeFile(path.join(facturas_data[id_fact].DIR, 'LOG', facturas_data[id_fact].ID + `DIAN.xml`), xml, "utf8", (err) => {
      if (err) throw console.log(err, 'error al escribir plano Log factura');
    });
  }
  guardarCufe() {
  ...
  async abrirArchivos_LOGS(dir, params) {
    if (params) {
      ...
      exe(params, function (err, data) {
        if (err); //console.log('Error ejecutando bat: \n\n' + err);
        else process.exit();
      });
    } else await exec(dir);
  }
  ...
  cerrarProcesosArchivos(tipo) {
    var instruccion = '';
    switch (tipo) {
      case 'excel': instruccion = 'taskkill /f /im excel.exe'; break;
      default: break;
    }
  ...
  validar_JSON(json) { validarJson(json) }
}
...
class _req_EKOMERCIO extends _req_FACT {
  ...
  async set_dataJson(obj) {
    let stringRest = '';
    let str_cabecera = '';
    let cabecera = obj.EKO['CABECERA'].split('|').map(e => {
      let cabecera = [];
      let item = e;
    ...
    let impuestos = obj.TOTALES['IMPUESTOS'].split('|').map(e => e.trim()).join('|');

    let retefuente = obj.TOTALES['RETEFUENTE'] ? obj.TOTALES['RETEFUENTE'].split('|').map(e => e.trim()).join('|') : '';

    let salud = obj.TOTALES['SALUD'] ? obj.TOTALES['SALUD'].split('|').map(e => e.trim()).join('|') : '';
    sal = obj.TOTALES['SALUD'] ? true : false; // AÑADE SALUD CUFE.BAT - EKOMERCIO

    let refer = obj.TOTALES['REFER'] ? obj.TOTALES['REFER'].split('|').map(e => e.trim()).join('|') : '';

    let ntc = obj.TOTALES['REFER'] ? true : false; // AÑADE EXCEPCION NOTA CREDITO - EKOMERCIO


    let descuentos = obj.TOTALES['DESCUENTOS'] ? obj.TOTALES['DESCUENTOS'].split('|').map(e => e.trim()).join('|') : '';

    let anticipos = obj.TOTALES['ANTICIPOS'] ? obj.TOTALES['ANTICIPOS'].split('|').map(e => e.trim()).join('|') : '';

    try {
      for (let campo of cabecera) str_cabecera += campo;//cabecera
      for (let campo of detalles) stringRest += campo['ITEMS'].split('|').map(e => e.trim()).join('|');//detallles
    ...
    } catch { return str_cabecera.concat(stringRest) };
    return str_cabecera.concat(stringRest).replace(/[\u0000-\u0019]+/g, "");
  }
...
class _req_EMISION extends _req_FACT {
  constructor(params) {
    ...
    this.url = params.nota_credito ? "https://endpoint.emision.co/api/v1/service/credit-note" : "https://endpoint.emision.co/api/v1/service/invoice";
    this.unidad = params.unidad;
    this.modulo = params.modulo;
  }
  ...
  send_request(data) {
    ...
    const options = {
      ...
      url: url,
      validateStatus: function (status) { return status != 500 || status != 400; }//Resolve solo si statusCode !=500
    }
...
const _factura_electronica = (params => {
  ...
  var
    proveedor = params.proveedor ? params.proveedor : 4,
    ...
    contabilidad = '\\' + localStorage.Contab, jsonText = '',
    nota_credito = params[4] == 'true' ? true : false,
    modulo = localStorage.Modulo,
    dataJson = params.dataJson, parseJSON = '';
  ...
  return new Promise(async function (resolve, reject) {
    ...
    switch (parseInt(proveedor)) {
      case 4://EKOMERCIO
        ...
        setTimeout(() => { resolve(facturas_logs) }, null);
        break;
      ...
      case 5://EMISION
        ...
        if (typeof req.validar_JSON(jsonText) == 'undefined') {
          ...
          setTimeout(() => { reject(req.validar_JSON(jsonText)['error']) }, null);
        }
        setTimeout(() => { resolve(facturas_logs) }, null);
        break;
...
const generarPDF_envios = (params => {
  ...
  var time = 0, date = new Date(), contador = 0, nombre;
  arrayPDF = new Array();
  ...
  return new Promise(async function (resolve, reject) {
    arrayPDF = [];
    switch (parseInt(proveedor)) {
      case 1: /*FACSE  e n  d e s a r r o l l o*/ break;
      case 4:// EKOMERCIO
        const servicio =
          tipo_envio == true
        ...
        nombre = `${nombre}\\${date.getFullYear().toString().concat(parseInt(date.getMonth()) + 1, (date.getDate()).toString().padStart(2, '0'), date.getHours(), date.getMinutes(), date.getSeconds())}`;
        nombre = nombre.concat(localStorage.Usuario, '.pdf');

        if (contador > 0) {
          try { merger.save(nombre); resolve(nombre); }
          catch (err) { reject(err) };
        } else reject({ error: 'El array se encuentra vacío: ' + arrayPDF });
        break;

      case 5:/* EMISION  e n  d e s a r r o l l o*/ break;
      default: break;
    }
  })
})
...
async function get_PDF(datos) {
  ...
  const options = {
    ...
    headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8', 'Expires': 200, 'cache-control': 'no-cache' },
    url: servicio,
    validateStatus: function (status) {
      return status != 500 || status != 400; // Resolve solo si statusCode !=500
    }
  ...
  console.time(time);
  const peticion = axios(options).then((result) => {
...
function abrir_PDF(ruta_pdf) { child(`start ${ruta_pdf}`); }
function createDir(dirPath) { fs.mkdir(dirPath, { recursive: true }, (err) => { if (err) return false; }) };

function respuesta_parser(obj) {
  var simpleObject = {};
  for (var prop in obj) {
    if (!obj.hasOwnProperty(prop)) continue;
    if (typeof (obj[prop]) == 'object') { if (prop != 'data') continue; }
